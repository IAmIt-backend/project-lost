...
namespace DB.Repositories
{
    public class DbThingRepository : IThingRepository
    {
        ...
        {
            return _thingCollection.AsQueryable().Where(t => t.About.ToLower().Contains(about.ToLower()) && t.ItemStatus.Equals(states)).ToList();
        }
        async public Task<List<Thing> > FindThingAsync(string about, ItemStates states)
        {
             return await _thingCollection.AsQueryable().Where(t => t.About.ToLower().Contains(about.ToLower()) && t.ItemStatus.Equals(states)).ToListAsync();
        }
